...
percent_practices <- Q2_practices %>%
                    ...
                    count() %>%
                    ungroup() %>%
                    left_join(n_site_practices, by = c("site", "gender")) %>%
  mutate(percentage = round(n*100/n_site,1))
  

top3_practices <- percent_practices %>%
  group_by(year, site, outcome_category, gender, indicator) %>%
...
effect_practices <- left_join(percent_practices, 
                        top3_practices, 
                        by = c("year","site","outcome_category", "practice_type", "indicator", "gender")) %>%
...
percent_services <- Q2_services %>%
  ...
  count() %>%
  ungroup() %>%
  left_join(n_site_services, by = c("site", "gender")) %>%
  mutate(percentage = round(n*100/n_site,1))
