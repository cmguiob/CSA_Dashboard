...
### Visualization choice: barplot with `ggplot` (CRAN)

According to Claus Wilke, humans are better perceiving distances over areas. Therefore, a barchart is decoded easier than a pie chart or a mosaic plot.

In the first chunk of this section the code creates a function to create a barplot for a location, year interval and CSA_category of choice.
...
if (!subtype %in% dataset$subtype) 
  ...
  filter(site == location, option == CSA_category, practice_type == type, subtype == subtype) %>%            select(year) %>%
  distinct()
...
p_barplot <- f_barplot(CSA_category = "Services", type = "Daily", subtype = "Only forecast", location = "Hoima", year_1 = 2018, year_2 = 2021)
