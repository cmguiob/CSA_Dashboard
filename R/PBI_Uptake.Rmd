...
### Visaulization choice: maps of selected sites
...
According to Claus Wilke, humans are better perceiving distances over areas. Therefore, a barchart is decoded easier than a pie chart or a mosaic plot. In this case, the bar segments are related to implementation and use cases (of practices and services) of farmers at a given site.
...
This is an alternative visualization option not used in PBI, but useful for the static reports, since it displays several 
...
The argumens of the function take in values similar to the drop down menus used in the PBI-dashboard. Although the `type` argument is optional it is recommended to use it, as it will improve the readability of the graphic; for the best experience, input a maximum of two types of practices. `subtype`and `top`are also optional: with the first it is possible to select specific practice sbtypes for the selected type, with the second it is possible to display the top 3 most implemented practices. If the argument `top = TRUE`, `type`and `subtype`must be ignored.
